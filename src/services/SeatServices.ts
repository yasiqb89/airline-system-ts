...
export async function releaseSeat(flightId: number, seatNumber: string, passengerId: number): Promise<Seat | null> {
    const pId = Number(passengerId);
    const fId = Number(flightId);
    ...
    if (flight.status === "departed" || flight.status === "cancelled" || flight.status === "boarding") return null;
    ...
    if (seat.reservedBy !== pId) return null;
